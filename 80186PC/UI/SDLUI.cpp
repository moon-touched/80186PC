...
SDLUI::SDLUI() : m_videoAdapter(nullptr) {
				...
				if (config.textMode) {
					...
					for (unsigned int row = 0; row < config.textModeRows; row++) {
						for (unsigned int column = 0; column < config.textModeColumns; column++) {
							...
							if (config.textModeCursorAddress == ptr - begin && config.textModeFirstCursorLine <= config.textModeLastCursorLine) {
								dst.x = x;
...
void SDLUI::pushKeyboardEvent(const SDL_KeyboardEvent& ev) {
	...
	if (code == 0x6A /* return */ && (ev.keysym.mod & KMOD_ALT)) {
		code = 0x04; // Enter
	}

	if (code == 0x2E /* keypad period */ && (ev.keysym.mod & KMOD_ALT)) {
		code = 0x35; // keypad comma
	}
	...
	std::unique_lock<std::mutex> locker(m_keyboardQueueMutex);
	m_keyboardQueue.push_back(code);
}
